...
def main():
    ...
    for episode in range(N_EPISODES):
        ...
        if episode % VERBOSITY_STEP == 0:
            ...
            #saver.append_epoch(episode, time_avg, score_avg, agent.epsilon, agent.memory.size())
            print("episode: {}/{}, time: {}, average scores: {}, e: {:.2}, memory: {} " \
                  .format(episode, N_EPISODES, time_avg, score_avg, agent.epsilon, agent.memory.size()))

            '''
            if episode % 100 == 0:
                print("model saved with avg score: ", score_avg)
                save_path = "saves/model" + str(episode) + ".h5"
